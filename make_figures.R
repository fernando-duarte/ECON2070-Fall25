...
if (Sys.getenv("FRED_API_KEY") == "") {
  message("Warning: FRED_API_KEY is not set. Set it with Sys.setenv(FRED_API_KEY='your_key') in R, or in .Renviron.")
}
...
ggsave("figures/fig_unemployment_long.pdf", p_unemp, width = 9, height = 4.8, units = "in")
...
ggsave("figures/fig_inflation_cpi.pdf", p_infl, width = 9, height = 4.8, units = "in")
...
gdp <- gdp |>
  mutate(fit_linear     = as.numeric(fitted(fit_lin)),
...
g_trend <- (p1 | p2) / (p3 | p4)
ggsave("figures/fig_gdp_trend_panels.pdf", g_trend, width = 10, height = 7.2, units = "in")
...
ggsave("figures/fig_gdp_detrended_overlay.pdf", p_detr, width = 9, height = 4.8, units = "in")

# Christiano-Fitzgerald bandpass components
cf <- mFilter::cffilter(gdp$logy, pl = 6, pu = 32, root = TRUE, drift = TRUE)
cf_df <- tibble(date = gdp$date,
...
ggsave("figures/fig_gdp_cffilter_components.pdf", g_cff, width = 9, height = 8.5, units = "in")
...
p_irf_ar <- ggplot(irf_ar, aes(h, irf, linetype = spec)) +
  ...
  labs(x = "Quarters", y = NULL, title = "AR impulse responses on detrended GDP") +
  theme_minimal() +
  theme(legend.position = "bottom")

ggsave("figures/fig_irf_ar_linear_vs_quadratic.pdf", p_irf_ar, width = 8, height = 4.5, units = "in")
...
cbi    <- fred_q("CBIC1")  |> rename(value = value) # change in private inventories, real
...
real_wage <- ahetpi |>
  rename(ahetpi = value) |>
  inner_join(cpi_q |> rename(cpi = value), by = "date") |>
...
qpanel <- gdp |>
  ...
  inner_join(pce |> rename(pce = value), by = "date") |>
  inner_join(gpdi |> rename(gpdi = value), by = "date") |>
  inner_join(cbi |> rename(cbi = value), by = "date") |>
  inner_join(expgs |> rename(expgs = value), by = "date") |>
  inner_join(gce |> rename(gce = value), by = "date") |>
  inner_join(payems |> rename(payems = value), by = "date") |>
  inner_join(awhman |> rename(awhman = value), by = "date") |>
  inner_join(real_wage |> rename(realw = value), by = "date") |>
  inner_join(gdpdef |> rename(gdpdef = value), by = "date") |>
  inner_join(ff_q |> rename(ff = value), by = "date") |>
  inner_join(gs10 |> rename(gs10 = value), by = "date") |>
  inner_join(sp500q |> rename(sp500 = value), by = "date") |>
  mutate(pi_gdpdef = 400 * (log(gdpdef) - log(lag(gdpdef, 1))),
         r10_expost = gs10 - lag(pi_gdpdef, 4)) |>
  drop_na()

# Cycle extraction with CF filter
cf_x <- function(x) mFilter::cffilter(log(x), pl = 6, pu = 32, root = TRUE, drift = TRUE)$cycle
cf_level <- function(x) mFilter::cffilter(x, pl = 6, pu = 32, root = TRUE, drift = TRUE)$cycle
...
ccf_row <- function(x, y, lags = lag_leads) {
  vapply(lags, function(ell) {
    if (ell >= 0) {
      stats::cor(x[(1+ell):length(x)], y[1:(length(y)-ell)], use = "pairwise.complete.obs")
    } else {
      ellp <- abs(ell)
      stats::cor(x[1:(length(x)-ellp)], y[(1+ellp):length(y)], use = "pairwise.complete.obs")
    }
  }, numeric(1))
}
...
m1     <- fred_m("M1SL", start = as.Date("1959-01-01"))
m2     <- fred_m("M2SL", start = as.Date("1959-01-01"))

monthly <- reduce(list(indpro, cpi_m, pcom, ff, totres, m1, m2),
                  ~full_join(.x, .y, by = "date"))

names(monthly) <- c("date", "indpro", "cpi", "pcom", "ff", "totres", "m1", "m2")
...
monthly <- monthly |>
  mutate(y = log(indpro),
         ...
         m2 = log(m2),
         ff = ff) |>
  select(date, y, p, pcom, ff, tr, m1, m2) |>
  drop_na()

# baseline sample avoiding zero lower bound period
monthly_sub <- monthly |>
  filter(date >= as.Date("1965-01-01"), date <= as.Date("2007-12-01"))

# estimate VAR with 12 lags
v <- vars::VAR(monthly_sub |>
                 select(y, p, pcom, ff, tr, m1, m2),
               p = 12, type = "const")
...
ir <- irf(v, impulse = "ff",
          response = c("y","p","pcom","tr","m1","m2","ff"),
...
tidy_ir <- function(ir_obj, var) {
  ...
  tibble(h = 1:length(point), irf = point, low = low, high = high, variable = var)
}

vars_list <- c("y","p","pcom","tr","m1","m2","ff")
irf_df <- bind_rows(lapply(vars_list, function(vname) tidy_ir(ir, vname)))

labels <- c(y = "Industrial production (log)",
            p = "CPI (log)",
...
p_irf <- ggplot(irf_df, aes(h, irf)) +
  ...
  labs(x = "Months after shock", y = NULL,
       title = "Impulse responses to a 100bp federal funds rate innovation") +
  theme_minimal()

ggsave("figures/fig_mp_irf_grid.pdf", p_irf, width = 10, height = 8.5, units = "in")
